...
import { View, StyleSheet, ScrollView, KeyboardAvoidingView, Platform, TouchableOpacity, Image, Text, SafeAreaView, Dimensions, Alert, PanResponder, Animated, Vibration } from 'react-native';
...
export const RegisterScreen: React.FC = () => {
  ...
  const [magnetTarget, setMagnetTarget] = useState<number | null>(null);
  ...
  const reorderPhotos = (fromIndex: number, toIndex: number) => {
    ...
    // Smooth sıralama güncellemesi - Diğer fotoğraflar sabit kalır
    ...
    // Sıralama değiştiğinde tüm fotoğrafları normale döndür
    setTimeout(() => {
      profilePhotos.forEach((_, i) => {
        if (dragAnimations[i]) {
          Animated.parallel([
            Animated.spring(dragAnimations[i].scale, {
              toValue: 1,
              tension: 300,
              friction: 10,
              useNativeDriver: true,
            }),
            Animated.timing(dragAnimations[i].opacity, {
              toValue: 1,
              duration: 150,
              useNativeDriver: true,
            }),
  ...
  // Magnet efekti için yardımcı fonksiyon
  const getMagnetTarget = (x: number, y: number): number | null => {
    const photoWidth = (width - spacing.lg * 2 - spacing.sm * 2) / 3;
    const photoHeight = photoWidth * 1.33;
    const totalWidth = photoWidth + spacing.sm;
    const totalHeight = photoHeight + spacing.sm;
    
    // Sadece mevcut grid'ler arasında magnet efekti
    const maxIndex = profilePhotos.length - 1;
    const col = Math.floor(x / totalWidth);
    const row = Math.floor(y / totalHeight);
    const targetIndex = row * 3 + col;
    
    // Mevcut grid'ler içinde mi kontrol et
  ...
  const handleDragStart = (index: number) => {
    ...
    // Tüm grid'leri yeşil yap - Yerleştirilebilir durumda
    setDropZoneIndex(-1); // Tüm grid'ler yeşil olacak
    setHoveredIndex(null); // Henüz hover yok
    setMagnetTarget(null); // Magnet target'ı sıfırla
    
    // Floating photo'yu göster - Modern ve profesyonel
    Animated.parallel([
    ...
    // Sadece seçilen fotoğrafı gizle - Diğerleri sabit kalır
    Animated.parallel([
  ...
  const handleDragEnd = () => {
    if (draggedIndex !== null) {
      // Floating photo'yu gizle - Modern ve profesyonel
      Animated.parallel([
        Animated.spring(floatingPhoto.scale, {
          toValue: 1,
          tension: 300,
      ...
      // Orijinal fotoğrafı normale döndür - Smooth geçiş
      Animated.parallel([
  ...
  // Advanced PanResponder for drag and drop
  const createPanResponder = (index: number) => {
    return PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        // Grid center pozisyonunu hesapla
        const col = index % 3;
        const row = Math.floor(index / 3);
        const gridCenterX = col * totalWidth;
        const gridCenterY = row * totalHeight;
        
        // Floating photo'yu grid ortasına yerleştir
      ...
      onPanResponderMove: (evt, gestureState) => {
        // Magnet efekti için pozisyon hesapla
        const photoWidth = (width - spacing.lg * 2 - spacing.sm * 2) / 3;
        const photoHeight = photoWidth * 1.33;
        const totalWidth = photoWidth + spacing.sm;
        const totalHeight = photoHeight + spacing.sm;
        
        // Magnet target'ı hesapla - sadece mevcut grid'ler arasında
        const magnetTargetIndex = getMagnetTarget(gestureState.moveX, gestureState.moveY);
        
        if (magnetTargetIndex !== null) {
          // Magnet efekti - grid'e yapış
          const col = magnetTargetIndex % 3;
          const row = Math.floor(magnetTargetIndex / 3);
          const gridCenterX = col * totalWidth;
          const gridCenterY = row * totalHeight;
          
          // Smooth magnet animasyonu
          Animated.timing(floatingPhoto.translateX, {
            toValue: gridCenterX,
            duration: 100,
            useNativeDriver: true,
          }).start();
          
          Animated.timing(floatingPhoto.translateY, {
            toValue: gridCenterY,
            duration: 100,
            useNativeDriver: true,
          }).start();
          ...
          // Magnet efekti yok - serbest hareket (sadece mevcut grid'ler içinde)
          const col = Math.floor(gestureState.moveX / totalWidth);
          const row = Math.floor(gestureState.moveY / totalHeight);
          const newIndex = Math.min(Math.max(0, row * 3 + col), profilePhotos.length - 1);
          
          // Sadece mevcut grid'ler içinde hareket et
          if (newIndex < profilePhotos.length) {
            const gridCenterX = col * totalWidth;
            const gridCenterY = row * totalHeight;
    ...
    <Animatable.View animation="fadeInUp" delay={400} style={styles.stepContent}>
      ...
      <View style={styles.photoGrid}>
          {profilePhotos.map((photo, index) => (
            <Animated.View 
              ...
              style={[
                ...
                // Sürüklenen fotoğraf sarı olmamalı - sadece hover durumu sarı
                draggedIndex !== index && hoveredIndex === index && styles.photoItemHovered,
                // Tüm grid'ler yeşil (dropZoneIndex === -1) - sürüklenen hariç
                (isDragging && dropZoneIndex === -1 && draggedIndex !== index) && styles.photoItemDropZone
              ]}
          ...
          {isDragging && draggedIndex !== null && (
            ...
            >
              <Image 
                source={{ uri: profilePhotos[draggedIndex] }} 
