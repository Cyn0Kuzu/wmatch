...
export class UserDataManager {
  ...
  public async startWatching(userId: string, movieData: UserMovieData): Promise<void> {
    try {
      ...
      await this.firestoreService.updateUserDocument(userId, { currentlyWatching });
  ...
  public async stopWatching(userId: string, movieId: number): Promise<void> {
    try {
      ...
      const currentlyWatching = userDoc.currentlyWatching || [];
      const filteredWatching = currentlyWatching.filter((item: any) => item.id !== movieId);
      
      await this.firestoreService.updateUserDocument(userId, { currentlyWatching: filteredWatching });
