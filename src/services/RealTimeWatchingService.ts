      ...
      if (Array.isArray(usersSnapshot)) {
        usersSnapshot.forEach((user: any) => {
          const profile: UserProfile = {
            ...
            photoURL: user.photoURL || user.profilePhotos?.[0] || null,
    ...
    try {
      ...
      for (const userWatching of allUsersWatching) {
        for (const watchingItem of userWatching.currentlyWatching || []) {
          ...
          const rawMovieId = watchingItem.movieId || watchingItem.id;
          const movieId = typeof rawMovieId === 'string' ? parseInt(rawMovieId, 10) : rawMovieId;
          const movieTitle = watchingItem.movieTitle || watchingItem.title || watchingItem.name;
          const moviePoster = watchingItem.moviePoster || watchingItem.poster || watchingItem.poster_path;
          const mediaType = watchingItem.media_type || watchingItem.type || 'movie';
          const genreIds = watchingItem.genre_ids || [];
          ...
          if (existingItem) {
              ...
              try {
                ...
                if (userDoc) {
                  userProfile = {
                    ...
                    photoURL: userDoc.photoURL || userDoc.profilePhotos?.[0] || null,
            ...
            if (userProfile) {
              const newItem: CurrentlyWatchingItem = {
                ...
                genre_ids: genreIds,
                viewers: [userProfile],
                currentTime: watchingItem.progress || watchingItem.currentTime || 0,
                totalDuration: 0,
